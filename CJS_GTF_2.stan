...
parameters {
  ...
  real<lower=0> Temp_thresh;                                                    // temperature threshold determining asymptotic behavior
  real log_beta_0;                                                              // log baseline survival/reporting
  real log_beta_D;                                                              // log depth effect
  real log_beta_T;                                                              // log temperature effect
}
...
transformed parameters {
  ...
  real beta_T = -exp(log_beta_T);                                               // force beta < 0 (monotone decrease)
  vector[N] f_D = (log1p_exp((Depth- Depth_thresh))-log1p_exp(-Depth_thresh));  // depth effect: scaled saturating function with max = 0
  vector[N] f_T = (log1p_exp((Temp- Temp_thresh))-log1p_exp(-Temp_thresh));     // Temp effect: scaled saturating function with max = 0
  vector[N] psi = exp(beta_0 + beta_D*f_D + beta_T*f_T);
...
model {
  // Priors
  alpha[1] ~ normal(-4,1);                                                      // intercept for encounter
  ...
  log_beta_T ~ normal(0,1);                                                     // prior of log-scale
  Depth_thresh ~ normal(20, 5);                                                 // depth threshold
  Temp_thresh ~ normal(25, 2.5);                                                // ecological threshold (e.g., 25°C)
  // ---- Likelihood ----
  for (i in 1:N) {
